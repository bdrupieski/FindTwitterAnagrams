...
object UpdateMatchMetrics extends TweetDatabase with MatchMetrics {
  def main(args: Array[String]) {
    ...
    val f = tweetsDb.stream(anagramMatchesTable.result).foreach(x => {
      val tweet1 = tweetsDb.run(tweetsTable.filter(y => y.id === x.tweet1Id).result)
      val tweet2 = tweetsDb.run(tweetsTable.filter(y => y.id === x.tweet2Id).result)
